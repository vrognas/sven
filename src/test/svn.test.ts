...
suite("Svn E2E Tests", () => {
  ...
  testIfReady(
    "1. Command spawn success - verify process execution",
    async function () {
      this.timeout(10000);
  ...
  testIfReady(
    "2. Auth failure - verify credential handling",
    async function () {
      this.timeout(10000);
  ...
  testIfReady(
    "3. Encoding detection - verify proper encoding conversion",
    async function () {
      this.timeout(10000);
      ...
      // Add and commit
      await svn.exec(checkoutDir.fsPath, ["add", testFile]);
      await svn.exec(checkoutDir.fsPath, [
        "commit",
        "-m",
        "Add non-ASCII file"
      ]);
