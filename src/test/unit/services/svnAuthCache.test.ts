...
suite("SvnAuthCache - Unit Tests", () => {
  ...
  suite("Credential File Writing", () => {
    test("1.1: writes credential file in correct SVN format", async () => {
      ...
      assert.strictEqual(
        cacheFiles.length,
        1,
        "Should write exactly one cache file"
      );
    ...
    test("1.2: sets file permissions to mode 600 on Unix", async function () {
      ...
      assert.strictEqual(
        mode,
        0o600,
        "File should have mode 600 (owner read/write only)"
      );
    });
    ...
    test("1.5: generates consistent filename for same realm+username", async () => {
      ...
      assert.strictEqual(
        filename1,
        filename2,
        "Should use same filename for same realm+username"
      );
    });
    ...
    test("1.6: overwrites existing credential file", async () => {
      ...
      assert.ok(
        updatedContent.includes(newPassword),
        "Should contain new password"
      );
      assert.ok(
        !updatedContent.includes(oldPassword),
        "Should not contain old password"
      );
    });
    ...
    test("1.7: handles special characters in username/password", async () => {
      ...
      assert.ok(
        ...
        "Should handle special chars in password"
      );
    });
    ...
    test("1.8: generates correct realm string from repository URL", async () => {
      ...
      for (const testCase of testCases) {
        ...
        assert.ok(
          content.includes("svn:realmstring"),
          `Should include realm for ${testCase.url}`
        );
      }
    });
  });
  ...
  suite("Credential File Reading", () => {
    ...
    test("2.2: returns null when credential file not found", async () => {
      const credentials = await authCache.readCredential(
        "https://nonexistent.com:443"
      );
      assert.strictEqual(
        credentials,
        null,
        "Should return null for non-existent realm"
      );
    });
  ...
  suite("Credential Cleanup", () => {
    ...
    test("3.3: deletes all cache files on dispose", async () => {
      await authCache.writeCredential(
        "user1",
        "pass1",
        "https://svn1.example.com:443"
      );
      await authCache.writeCredential(
        "user2",
        "pass2",
        "https://svn2.example.com:443"
      );
      ...
      files.forEach(file => {
        assert.ok(
          !fs.existsSync(file),
          `File ${file} should be deleted on dispose`
        );
      });
    });
  ...
  suite("Error Handling", () => {
    test("4.1: handles write failure due to permission denied", async function () {
      ...
      try {
        ...
        assert.ok(
          err.message.includes("permission") || err.message.includes("EACCES")
        );
      } finally {
    ...
    test("4.4: handles empty credential file", async () => {
      ...
      assert.strictEqual(
        credentials,
        null,
        "Should return null for empty file"
      );
    });
  ...
  suite("Concurrent Access", () => {
    test("5.1: handles concurrent writes to different realms", async () => {
      ...
      assert.strictEqual(
        files.length,
        3,
        "Should create 3 separate cache files"
      );
    });
  ...
  suite("Cross-Platform Paths", () => {
    ...
    test("6.4: normalizes path separators for current platform", async () => {
      ...
      assert.ok(
        filePath.includes(separator),
        `Should use ${separator} as path separator`
      );
    });
  });
});
