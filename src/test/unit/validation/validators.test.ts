...
suite("Validation Tests", () => {
  ...
  suite("validateRepositoryUrl", () => {
    test("accepts valid SVN URLs", () => {
      assert.strictEqual(
        validateRepositoryUrl("http://svn.example.com/repo"),
        true
      );
      assert.strictEqual(
        validateRepositoryUrl("https://svn.example.com/repo"),
        true
      );
      assert.strictEqual(validateRepositoryUrl("svn://server/repo"), true);
      assert.strictEqual(
        validateRepositoryUrl("svn+ssh://user@server/repo"),
        true
      );
    });
    ...
    test("rejects dangerous protocols", () => {
      ...
      assert.strictEqual(
        validateRepositoryUrl("data:text/html,<script>"),
        false
      );
    });
    ...
    test("rejects command injection in hostname", () => {
      assert.strictEqual(
        validateRepositoryUrl("http://server;rm-rf/repo"),
        false
      );
      assert.strictEqual(
        validateRepositoryUrl("http://server&&echo/repo"),
        false
      );
      assert.strictEqual(
        validateRepositoryUrl("http://server`whoami`/repo"),
        false
      );
      assert.strictEqual(
        validateRepositoryUrl("http://server$(cat)/repo"),
        false
      );
    });
    ...
    test("rejects command injection in path", () => {
      assert.strictEqual(
        validateRepositoryUrl("http://server/repo;rm-rf"),
        false
      );
      assert.strictEqual(
        validateRepositoryUrl("http://server/repo&&echo"),
        false
      );
      assert.strictEqual(
        validateRepositoryUrl("http://server/repo`cat`"),
        false
      );
      assert.strictEqual(
        validateRepositoryUrl("http://server/repo$(ls)"),
        false
      );
    });
    ...
    test("accepts URLs with ports and auth", () => {
      assert.strictEqual(
        validateRepositoryUrl("http://server:8080/repo"),
        true
      );
      assert.strictEqual(
        validateRepositoryUrl("https://user:pass@server/repo"),
        true
      );
      assert.strictEqual(
        validateRepositoryUrl("svn+ssh://user@server:22/repo"),
        true
      );
    });
    ...
    test("accepts URLs with query params", () => {
      assert.strictEqual(
        validateRepositoryUrl("http://server/repo?param=value"),
        true
      );
      assert.strictEqual(
        validateRepositoryUrl("https://server/repo?a=1&b=2"),
        true
      );
    });
  ...
  suite("validateLockComment", () => {
    test("accepts valid comments", () => {
      assert.strictEqual(validateLockComment("Editing data file"), true);
      assert.strictEqual(validateLockComment("Work in progress"), true);
      assert.strictEqual(validateLockComment("Bug fix #123"), true);
    });

    test("accepts empty string", () => {
      assert.strictEqual(validateLockComment(""), true);
    });

    test("rejects non-string input", () => {
      assert.strictEqual(validateLockComment(null as any), false);
      assert.strictEqual(validateLockComment(undefined as any), false);
      assert.strictEqual(validateLockComment(123 as any), false);
    });

    test("rejects shell metacharacters", () => {
      assert.strictEqual(validateLockComment("test; rm -rf"), false);
      assert.strictEqual(validateLockComment("test && echo"), false);
      assert.strictEqual(validateLockComment("test | grep"), false);
      assert.strictEqual(validateLockComment("test $(cat)"), false);
      assert.strictEqual(validateLockComment("test `whoami`"), false);
      assert.strictEqual(validateLockComment("test { }"), false);
      assert.strictEqual(validateLockComment("test()"), false);
      assert.strictEqual(validateLockComment("test\\n"), false);
    });

    test("accepts normal punctuation", () => {
      assert.strictEqual(validateLockComment("It's working!"), true);
      assert.strictEqual(validateLockComment("Fix: issue #123"), true);
      assert.strictEqual(validateLockComment("Update [draft]"), true);
      assert.strictEqual(validateLockComment("Version 1.0.0"), true);
    });
  });
});
